...
using namespace ns3;
using namespace std;
NS_LOG_COMPONENT_DEFINE("lte-my-multi-example");

class MyCallBacks {
public:
	static void ConnectionEstablishedCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti);
	static void ConnectionReestablishedCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti);
	static void ConnectionReestablishedRejectedCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti);
	static void ConnectionTimeoutCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti);

};

void MyCallBacks::ConnectionEstablishedCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti) {
	cout << "ConnectionEstablishedCallback,(imsi - cellid)," << imsi << "," << cellId << endl;

}

void MyCallBacks::ConnectionReestablishedCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti) {
	cout << "ConnectionReestablishedCallback,(imsi - cellid)," << imsi << "," << cellId << endl;

}

void MyCallBacks::ConnectionReestablishedRejectedCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti) {
	cout << "ConnectionReestablishedRejectedCallback,(imsi - cellid)," << imsi << "," << cellId << endl;

}

void MyCallBacks::ConnectionTimeoutCallback(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti) {
	cout << "ConnectionTimeoutCallback,(imsi - cellid)," << imsi << "," << cellId << endl;
}


/**
 * Get the cell position (x, y, z=0) based on the radius and the hexagonal grid size.
 * The hexagonal grid starts with the center cell with id=0, and continue with
 * the first and second tier.
 *
 * @param nCells the number of cells in the hexagonal grid
 * @param radius the cell radius
 * @return all the positions from all cells
 */
vector<Vector> getCellsPosition(int nCells, double radius) {
...
int main(int argc, char *argv[]) {
	...
	//System
	double antBeamwidth = 90.0; //degree
	...
	int srsPeriodicity = 320; //Allowed values: 2 5 10 20 40=default 80 160 320
	...
	int nCells = 1;
	int nSectors = 3; //if 1, the antenna wilsimSeedl be omnidirectional.
	...
	vector<Vector> cellsPosit = getCellsPosition(nCells, cellRadius);
	...
	for (int i = 0; i < nCells; i++) {
		for (int j = 0; j < nSectors; j++) {
			Ptr<LteEnbNetDevice> dev = enbNetDevCont.Get(i * nSectors + j)->GetObject<LteEnbNetDevice> ();
	...
	Config::Connect("/NodeList/*/DeviceList/*/LteUeRrc/ConnectionEstablished", MakeCallback(&MyCallBacks::ConnectionEstablishedCallback));
	Config::Connect("/NodeList/*/DeviceList/*/LteUeRrc/ConnectionTimeout", MakeCallback(&MyCallBacks::ConnectionEstablishedCallback));
	Config::Connect("/NodeList/*/DeviceList/*/LteUeRrc/ConnectionReestablished", MakeCallback(&MyCallBacks::ConnectionReestablishedCallback));
	Config::Connect("/NodeList/*/DeviceList/*/LteUeRrc/ConnectionReestablishedRejected", MakeCallback(&MyCallBacks::ConnectionReestablishedRejectedCallback));
