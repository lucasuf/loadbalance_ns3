...
void
LteHelper::AttachRXPowerEnb (NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
  NS_LOG_FUNCTION (this);
  int k=0;
  for (NetDeviceContainer::Iterator i = ueDevices.Begin (); i != ueDevices.End (); ++i)
    {
	  NS_LOG_LOGIC ("Checking UE(" << k << ")");

	  AttachRXPowerEnb (*i, enbDevices);
	  k++;
    }
}

void
LteHelper::AttachRXPowerEnb (Ptr<NetDevice> ueDevice, NetDeviceContainer enbDevices)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (enbDevices.GetN () > 0, "empty enb device container");
  double minGain = std::numeric_limits<double>::infinity () * (-1.0);
  Ptr<MobilityModel> ueMobModel = ueDevice->GetNode ()->GetObject<MobilityModel> ();
  Vector uepos = ueMobModel->GetPosition ();
  ...
  for (NetDeviceContainer::Iterator i = enbDevices.Begin (); i != enbDevices.End (); ++i)
    {
	  Ptr<LteEnbNetDevice> dev = (*i)->GetObject<LteEnbNetDevice>();
	  Ptr<MobilityModel> enbMobModel = (*i)->GetNode ()->GetObject<MobilityModel> ();
	  Vector enbpos = enbMobModel->GetPosition ();
	  distance = CalculateDistance (uepos, enbpos);
	  //vecDist.push_back(distance);
	  NS_LOG_LOGIC ("Distance eNB(" << k << "): " << distance);

      Ptr<LteEnbPhy> enbPhy = dev->GetPhy();
      //double txPower = enbPhy->GetTxPower();
      Ptr<AntennaModel> antennaENB = enbPhy->GetDownlinkSpectrumPhy()->GetRxAntenna();
      Angles txAngles(uepos, enbpos);
      txAntennaGain = antennaENB->GetGainDb(txAngles);
      //vecGain.push_back(txAntennaGain);
      NS_LOG_LOGIC ("Gain eNB(" << k << "): " << txAntennaGain);

      if (distance <= maxDist) {
			if (txAntennaGain > minGain) {
				NS_LOG_LOGIC("Selecting eNB " << k << "  Gain: " << txAntennaGain << ", D=" << distance);
				minGain = txAntennaGain;
				closestEnbDevice = *i;
				kSelected = k;
			}
			maxDist = distance;
...
//      Ptr<AntennaModel> antennaENB = enbPhy->GetDownlinkSpectrumPhy()->GetRxAntenna();
//      Angles txAngles(uepos, enbpos);
//      double txAntennaGain = antennaENB->GetGainDb(txAngles);
//      NS_LOG_LOGIC ("Gain eNB(" << k << "): " << txAntennaGain);

//      Ptr<SpectrumValue> svalue = enbPhy->CreateTxPowerSpectralDensity();
//      svalue->Add(txPower);
//      Ptr<SpectrumPropagationLossModel> dlSplm = m_downlinkPathlossModel->GetObject<SpectrumPropagationLossModel> ();
//      Ptr<SpectrumValue> propagationGainDb = dlSplm->CalcRxPowerSpectralDensity(svalue, enbMobModel, ueMobModel);
//      double s = Sum(*propagationGainDb);
//	  cout << "Prx eNB(" << k << "): " << s << endl;
...
//        }
  ...
  NS_LOG_LOGIC("Selected eNB " << kSelected << "!!!!");
