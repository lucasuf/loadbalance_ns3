...
MYPATH=output/sim
MYBIN=lte-my-multi-example

	for nuebe in "${NBUE_BE[@]}"
	do
		for nuemm in "${NBUE_MM[@]}"
		do
			time ./waf --run src/lte/examples/$MYBIN --command-template="%s --ns3::ConfigStore::Filename=input-my-multi.txt --ns3::ConfigStore::Mode=Load --ns3::ConfigStore::FileFormat=RawText --numUesBE=${nuebe} --numUesVoIP=${nuemm} --nCells=$NCELLS --cellRadius=$CELLRADIUS --nSectors=$NSECTORS --ueDistrRadius=$UEDISTRADIUS --interPacketInterval=$PKGINT --maxPacketSize=$PKGSIZE"
